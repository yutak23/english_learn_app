...
**注意:**
- ユーザー向けの表示では「Study（学習）」のみを使用します。
- 内部実装（FSRS）では技術的な状態として「Review」が存在しますが、これはアルゴリズム内部の用語であり、ユーザーには表示しません。
- Report画面では、FSRS内部の状態を「New / Learning / Stable」の3つの表示状態にマッピングします（詳細は DESIGN.md を参照）。
...
| 音声 | スピーカーアイコンで発音再生 |
| 回答ボタン | `Forgot`（赤）、`Perfect`（オレンジ）、`Remembered`（青）<br>※ セット内で `Forgot` 選択後は `Perfect` 非表示 |
...
| セクション | 内容 |
|-----------|------|
| Overview | 総学習日数、総学習回数、総学習単語数、ストリーク、ストレージ使用量 |
| Daily Studies | 日別学習数（期間フィルタ、グラフ表示） |
| Word Status | 全単語の学習状態（New, Learning, Stable）、ソート機能 |
